...
int cmps12_read_register(int file, uint8_t register_addr, uint8_t *data,
                         ssize_t data_size) {
...
int cmps12_write_register(int file, uint8_t register_addr, uint8_t data,
                          ssize_t data_size) {
...
uint8_t cmps12_read_bearing_8_bit(int cmps12_file) {
  ...
  if (cmps12_read_register(cmps12_file, COMPASS_BEARING_8_BIT_REG,
                           &bearing_8bit, sizeof(bearing_8bit)) < 0) {
    fprintf(stderr, "failed to read compas bearing 8 bit\n");
    return 0;
  }

  float bearing_degrees = (bearing_8bit * 360) / 255.0;

  bearing_degrees = fmod(bearing_degrees, 360.0);
...
uint8_t cmps12_read_calibration_state(int cmps12_file) {
  ...
  if (cmps12_read_register(cmps12_file, CALIBRATION_REG, &calibration_state,
                           sizeof(calibration_state)) < 0) {
...
uint16_t cmps12_read_bearing_16_bit_quaternion(int cmps12_file) {
  ...
  // read high byte
  if (cmps12_read_register(cmps12_file,
                           COMPASS_BEARING_16_BIT_REG_HIGH_QUATERNION,
                           &high_byte, sizeof(high_byte)) < 0) {
  ...
  // read low byte
  if (cmps12_read_register(cmps12_file,
                           COMPASSS_BEARING_16_BIT_REG_LOW_QUATERNION,
                           &low_byte, sizeof(low_byte)) < 0) {
  ...
  float bearing_degrees = bearing_16_bit / (1 << 12) * 359.9;
...
int8_t cmps12_read_pitch_90_degress(int cmps12_file) {
  ...
  if (cmps12_read_register(cmps12_file, PITCH_ANGLE_REG_90_DEGRESS,
                           &pitch_90_degress, sizeof(pitch_90_degress)) < 0) {
...
int8_t cmps12_read_roll_90_degress(int cmps12_file) {
  ...
  if (cmps12_read_register(cmps12_file, ROLL_ANGLE_REG_90_DEGRESS,
                           &roll_90_degress, sizeof(roll_90_degress)) < 0) {
...
uint16_t cmps12_read_mag_x_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, MAG_X_RAW_16_BIT_REG_HIGH, &high_byte,
                           sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, MAG_X_RAW_16_BIT_REG_LOW, &low_byte,
                           sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_mag_y_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, MAG_Y_RAW_16_BIT_REG_HIGH, &high_byte,
                           sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, MAG_Y_RAW_16_BIT_REG_LOW, &low_byte,
                           sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_mag_z_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, MAG_Z_RAW_16_BIT_REG_HIGH, &high_byte,
                           sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, MAG_Z_RAW_16_BIT_REG_LOW, &low_byte,
                           sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_accelerometer_x_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, ACCELEROMETER_X_RAW_16_BIT_HIGH,
                           &high_byte, sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, ACCELEROMETER_X_RAW_16_BIT_LOW,
                           &low_byte, sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_accelerometer_y_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, ACCELEROMETER_Y_RAW_16_BIT_HIGH,
                           &high_byte, sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, ACCELEROMETER_Y_RAW_16_BIT_LOW,
                           &low_byte, sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_accelerometer_z_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, ACCELEROMETER_Z_RAW_16_BIT_HIGH,
                           &high_byte, sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, ACCELEROMETER_Z_RAW_16_BIT_LOW,
                           &low_byte, sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_gyro_x_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, GYRO_X_RAW_16_BIT_HIGH, &high_byte,
                           sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, GYRO_X_RAW_16_BIT_LOW, &low_byte,
                           sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_gyro_y_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, GYRO_Y_RAW_16_BIT_HIGH, &high_byte,
                           sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, GYRO_Y_RAW_16_BIT_LOW, &low_byte,
                           sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_gyro_z_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, GYRO_Z_RAW_16_BIT_HIGH, &high_byte,
                           sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, GYRO_Z_RAW_16_BIT_LOW, &low_byte,
                           sizeof(low_byte)) < 0) {
...
uint16_t cmps12_temperature_16_bit(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, TEMP_REG_16_BIT_HIGH, &high_byte,
                           sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, TEMP_REG_16_BIT_LOW, &low_byte,
                           sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_bearing_16_bit_BNO055(int cmps12_file) {
  ...
  if (cmps12_read_register(cmps12_file, COMPASS_BEARING_16_BIT_REG_HIGH_BNO055,
                           &high_byte, sizeof(high_byte)) < 0) {
  ...
  if (cmps12_read_register(cmps12_file, COMPASSS_BEARING_16_BIT_REG_LOW_BNO055,
                           &low_byte, sizeof(low_byte)) < 0) {
...
uint16_t cmps12_read_pitch_180_degress(int cmps12_file) {
  ...
  // read the high byte
  if (cmps12_read_register(cmps12_file, PITCH_ANGLE_16_BIT_REG_HIGH_180_DEGRESS,
                           &high_byte, sizeof(high_byte)) < 0) {
  ...
  // read the low_byte
  if (cmps12_read_register(cmps12_file, PITCH_ANGLE_16_BIT_REG_LOW_180_DEGRESS,
                           &low_byte, sizeof(low_byte)) < 0) {
